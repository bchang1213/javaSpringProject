	...
	<div class="container-fluid">
		...
		<main>
			<div class="friendslist">
				...
				<a href="/profile/${friend.getId()}/${friend.getAlias()}">${friend.getFirst_name()} ${friend.getLast_name()}</a>
				</c:forEach>
			</div>
			<div class="playlist">
				<h5>Your Playlist</h5>
				<c:forEach items = "${userPlaylist.getVideos()}" var = "video">
					<div>
						<a href="/library/${video.getId()}">
							<img class="vid-item" src="${video.getThumbnail_url()}" alt="${video.getTitle()}thumb"/>
						</a>
						<div></div>
						<a href="/library/${video.getId()}">${video.getTitle()}</a>
					</div>
				</c:forEach>
			</div>
		</main>
